...
บ                             Bob Ray Tracer                              บ
บ                                                                         บ
บ   PreProc.C = preprocessor for Bob.  Handles macros and include files.  บ
บ                                                                         บ
...
int     preproc(char *infile, char *outfile)
{
...
void    add_macro(char *txt)
{
	...
	while(1) {
		...
		if(*txt == '\\') {      /* continuation character? */
			...
			txt++; *txt = ' '; txt++;       /* put a blank between lines */
			cptr = line;
...
char    *brute(char *text, char *pat, int tlen, int plen)
{
	register int     i, j;
